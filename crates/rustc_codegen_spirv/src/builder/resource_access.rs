        ...
        match self.lookup_type(result_type) {
            SpirvType::Pointer { pointee } => match self.lookup_type(pointee) {
                SpirvType::Sampler => {
                    ...
                    self
                        .emit()
                        .access_chain(result_type, None, set, vec![bindless_idx])
                        .unwrap()
                        .with_type(result_type)
                }
                SpirvType::Image { dim, sampled, .. } => {
                    let set = self.cx.texture_bindless_descriptor_set(pointee, dim, sampled);
                    self
                        .emit()
                        .access_chain(result_type, None, set, vec![bindless_idx])
                        .unwrap()
            ...
            ty => self.fatal(&format!("Non-Pointer type {:?} unsupported for resource access", ty)),
        }
    }
}
